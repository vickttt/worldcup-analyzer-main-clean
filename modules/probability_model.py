...
def combine_probabilities(odds, polymarket, news, config):
    ...
    if has_odds:
        ...
        odds_probs = normalize_probabilities({
    ...
    unused_market_weight = 1 - odds_weight - polymarket_weight - news_weight
    fallback_probs = market_probs if has_polymarket else odds_probs
    for key in ["home_win", "draw", "away_win"]:
        combined[key] += fallback_probs[key] * unused_market_weight
